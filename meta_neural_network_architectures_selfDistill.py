...
class VGGReLUNormNetwork(nn.Module):
    ...
    def build_network(self):
        ...
        for i in range(self.num_stages):
            ...
            # 추가
            middle_out = out
            middle_out = middle_out.view(middle_out.shape[0], -1)
            self.layer_dict['fc{}'.format(i)] = MetaLinearLayer(input_shape=(middle_out.shape[0], np.prod(middle_out.shape[1:])),
    ...
    def forward(self, x, num_step, params=None, training=False, backup_running_statistics=False):
        ...
        for i in range(self.num_stages):
            ...
            middle_out = out
            middle_out = middle_out.view(middle_out.shape[0], -1)
            middle_out = self.layer_dict['fc{}'.format(i)](middle_out, param_dict['fc{}'.format(i)])
            middle_out_list.append(middle_out)
