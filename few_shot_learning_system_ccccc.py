...
from loss import knowledge_distillation_loss, LabelSmoothingCrossEntropy
...
class MAMLFewShotClassifier(nn.Module):
    def __init__(self, im_shape, device, args):
        ...
        # self.M = self.get_meta_parameters(self.classifier.named_parameters(), device=self.device)
        ...
        if self.args.arbiter:
            ...
            # self.arbiter = Arbiter(input_dim=input_dim, output_dim=output_dim, M=self.M, args=self.args,
            #                                 device=self.device)
        ...
        self.optimizer = SAM(self.trainable_parameters(), base_optimizer, lr=args.meta_learning_rate)
    ...
    def get_meta_parameters(self, named_parameters, device):
    ...
    def get_across_task_loss_metrics(self, total_losses, total_accuracies):
        ...
        # task1_gradient = task_gradients[0]['layer_dict.linear.weights'].detach().clone()
        # task2_gradient = task_gradients[1]['layer_dict.linear.weights'].detach().clone()
    ...
    def forward(self, data_batch, epoch, use_second_order, use_multi_step_loss_optimization, num_steps, training_phase, current_iter):
        ...
        for task_id, (x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in enumerate(zip(x_support_set,
            ...
            for num_step in range(num_steps):
                ...
                ## Dynamic ClassWeighting Scheme for Classification Tasks
                # print("before support_loss == ", torch.mean(support_loss))
                per_classes_loss_weight = F.softmax(support_loss, dim=0)
                weighted_support_loss = torch.sum(per_classes_loss_weight * support_loss)
                ...
                if self.args.arbiter:
                    ...
                    # 수정
                    support_loss_grad = torch.autograd.grad(weighted_support_loss, names_weights_copy.values(),
                                                            retain_graph=True)
                    ...
                    # generated_gradient_rate = self.arbiter(task_state=per_step_task_embedding)
                ...
                names_weights_copy = self.apply_inner_loop_update(loss=weighted_support_loss,
                ...
                if use_multi_step_loss_optimization and training_phase and epoch < self.args.multi_step_loss_num_epochs:
                    ...
                    per_classes_loss_weight = F.softmax(target_loss, dim=0)
                    weighted_target_loss = torch.sum(per_classes_loss_weight * target_loss)
                    task_losses.append(weighted_target_loss)
            ## Inner-loop END
    ...
    def net_forward(self, x, y, weights, backup_running_statistics, training, num_step, training_phase, epoch, soft_target=None):
        ...
        if training_phase:
            if self.args.smoothing:
                criterion = LabelSmoothingCrossEntropy(smoothing=0.1, reduction='none')
    ...
    def calculate_weight_difference(self, meta_learner, base_learner, lambda_diff=1.0):

        total_diff_norm = 0.0

        # Arbiter는 제외
        for key in meta_learner.keys():
            if not 'linear' in key:
                diff_norm = torch.norm(meta_learner[key] - base_learner[key].squeeze(), p=2)
                total_diff_norm = total_diff_norm - diff_norm

        # for key in meta_learner.keys():
        #     if 'linear' in key:
        #         # diff_norm = F.mse_loss(weight1[key], weight2[key], reduction='sum')
        #         # total_diff_norm = total_diff_norm + diff_norm
        #
        #         # 가중치의 차이에 대한 L2 Norm
        #         diff_norm = torch.norm(meta_learner[key] - base_learner[key].squeeze(), p=2)
        ...
        # print("total_diff_norm == ", total_diff_norm)

        return lambda_diff * total_diff_norm
    ...
    def meta_update(self, loss, current_iter, first_step, epoch, sharpness_measure=None):
        """
    ...
    def run_train_iter(self, data_batch, epoch, current_iter):
        ...
        sharpness_measure.append(losses_1)
        ...
        sharpness_measure.append(losses)

        self.meta_update(loss=losses['loss'], current_iter=current_iter, first_step=False, epoch=epoch, sharpness_measure=sharpness_measure)
