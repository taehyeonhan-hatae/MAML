...
from meta_neural_network_architectures_selfDistill import VGGReLUNormNetwork,ResNet12, StepArbiter, Arbiter
from inner_loop_optimizers_selfDistill import GradientDescentLearningRule, LSLRGradientDescentLearningRule

from timm.loss import LabelSmoothingCrossEntropy
from loss import knowledge_distillation_loss, KL_divergence
...
class MAMLFewShotClassifier(nn.Module):
    ...
    def net_forward(self, x, y, weights, backup_running_statistics, training, num_step, training_phase, epoch, soft_target=None):
        ...
        preds, middle_out_list = self.classifier.forward(x=x, params=weights,
                                        training=training,
        ...
        # for shallow classifier의 logits distillation
        teacher_output = preds.detach()
        for index in range(len(middle_out_list)):
            loss_coefficient = 0.9

            loss = loss + KL_divergence(student_logit=middle_out_list[index], teacher_logit=teacher_output, Temperature=1.0) * loss_coefficient
            loss = loss + F.cross_entropy(input=middle_out_list[index], target=y) * (1-loss_coefficient)
